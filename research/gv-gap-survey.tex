...
\newenvironment{paperquote}{%
  ...
  \item\relax\small\itshape\leavevmode%
  \hspace{-1.3em}\textcolor{gray!80}{\rule{2pt}{1em}}\hspace{0.5em}%
}{%
  \end{list}%
  \vspace{0.3em}%
}
...
This report covers the \textbf{generator--validator (GV) gap} in large language models: the asymmetry between a model's ability to \emph{generate} correct outputs and its (typically superior) ability to \emph{validate or discriminate} among candidate outputs. The gap was named and formalized in two concurrent papers:

\begin{paperquote}
\small ``One clear manifestation of this is the generator-validator gap, where a model may generate answers that it does not verify as correct, or vice versa. Resolving this inconsistency would lead to LLMs that report their uncertainty accurately and are less susceptible to hallucination.''\\
\hfill --- Rodriguez et al.\ (RankAlign, 2025)
\end{paperquote}

\begin{paperquote}
\small ``Even state-of-the-art language models suffer from inconsistency, which undermines their reliability.''\\
\hfill --- Li et al.\ (Benchmarking GV Consistency, 2023)
\end{paperquote} We organized the literature into ten focused Zotero subcollections under \texttt{LLM-inconsistency}, each targeting a specific research question. Papers were retained only when directly relevant; marginal items were removed after a relevance audit. Detailed per-paper notes are attached as child notes in Zotero and in \texttt{gv-q1-detailed-dossier.md} and the per-question \texttt{gv-q*.md} files.
...
This paper introduces GSM8K (8.5K grade-school math problems) and proposes training a separate learned verifier to score candidate solutions sampled from a generator; the highest-scoring candidate is selected. The key empirical claim:

\begin{paperquote}
\small ``We show that, compared to a finetuning baseline, the use of verifiers results in approximately the same performance boost as a 30x model size increase, and that verifiers scale significantly better with increased data.''\\
\hfill --- Cobbe et al.\ (2021)
\end{paperquote} The paper establishes that candidate diversity combined with a quality filter is a practically viable lever for boosting generation quality without changing the generator's weights. The main caveat is domain specificity: the verifier is trained on math and needs re-training or re-calibration for other domains. The central open question is how verifier quality and candidate sample count trade off at fixed compute budget.

\paragraph{Solving Math Word Problems With Process- and Outcome-Based Feedback (Uesato et al., 2022).}
This paper provides a rigorous comparison of \emph{process-based} versus \emph{outcome-based} feedback signals for math reasoning (GSM8K), using large-scale language models (including a Base-70B configuration). They study SFT, outcome reward modeling (ORM), process reward modeling (PRM), and expert-iteration-style RL variants. Key reported numbers: final-answer error falls from 16.8\% to 12.7\%, trace error from 14.0\% to 3.4\% (best models), and with 30\% abstention, final-answer error reaches 2.7\%. Process-aware validators improve both final-answer correctness and reasoning trace quality; outcome-only supervision is consistently inferior (23.5\% vs 22.3\% error, without reward models; 16.6\% vs 14.8\%, with). This paper directly shows that validator \emph{design} matters: a step-level critic improves generator outputs beyond what a final-answer judge can. The main limitation is annotation cost for step-level labels.

\paragraph{Let's Verify Step by Step (Lightman et al., 2023).}
This paper scales process supervision with PRM800K, a dataset of 800,000 human step-level labels for math reasoning (MATH benchmark). They compare outcome-supervised reward models (ORMs) versus process-supervised reward models (PRMs):

\begin{paperquote}
\small ``[We find] that process supervision significantly outperforms outcome supervision for training models to solve problems from the challenging MATH dataset. Our process-supervised model solves 78\% of problems from a representative subset of the MATH test set.''\\
\hfill --- Lightman et al.\ (2023)
\end{paperquote} Active learning gives a 2.6$\times$ data efficiency gain for collecting process labels. This is the strongest evidence that a high-quality step-level validator materially improves generation selection. The main limitation is the high annotation cost and domain concentration in math; open questions concern whether synthetic step labels can replace part of PRM800K.
...
This paper probes whether LLMs have reliable metacognitive signals about their own correctness. Using P(True)-style elicitation (prompting the model to judge whether its answer is correct), it finds that confidence estimates are \emph{substantially more accurate than first-pass generation suggests}. Crucially for the GV gap:

\begin{paperquote}
\small ``[W]e find encouraging performance, calibration, and scaling for P(True) on a diverse array of tasks. \ldots\ [W]e provide evidence that verification improves faster than generation quality in this context.''\\
\hfill --- Kadavath et al.\ (2022)
\end{paperquote}

This is a core mechanism paper for the GV gap: the latent belief is present, but the readout (generation) is imperfect.
...
This paper trains a lightweight classifier on LLM hidden states to detect deceptive or incorrect statements. Even when the model's output is wrong, the internal state carries reliable truth/deception-predictive signals. The implication for GV gap is that discriminative tasks (validation) may better exploit these internal signals than free generation, though the exact mechanism by which generation fails to surface them is not directly tested in this paper and remains an open question.
...
Sycophancy---where a model changes its stated position to match user-expressed preferences---is a direct mechanism for GV gap: the generator produces preference-aligned but not truth-aligned outputs. This paper shows that user-expressed opinions systematically shift model outputs toward social agreement over accuracy, and that this behavior is incentivized by RLHF/preference optimization training. The connection to GV gap is that sycophancy can suppress correct generation even when the model's internal state carries reliable truth signal (as shown by Kadavath et al. and Burns et al. above).
...
This founding paper shows that human preferences over short trajectory clips can be used to train reward models that then drive RL policy improvement, without any task-specific reward engineering. In one reported example, $\sim$1,300 human comparisons suffice to teach an Atari Enduro agent to match target behavior; locomotion tasks are also evaluated with similarly small comparison budgets. This establishes the basic \emph{validator-as-reward-model} paradigm that all subsequent RLHF work inherits.
...
Generate diverse reasoning chains via temperature sampling, then take a majority vote over final answers. The paper states results directly:

\begin{paperquote}
\small ``[S]elf-consistency improves over chain-of-thought prompting by a striking margin across all tasks. In particular, when used with PaLM-540B or GPT-3, self-consistency achieves new state-of-the-art levels of performance across arithmetic reasoning tasks, including GSM8K (+17.9\%), SVAMP (+11.0\%), AQuA (+12.2\%).''\\
\hfill --- Wang et al.\ (2022)
\end{paperquote}

The mechanism: when generator errors are random and diverse, majority voting exploits the model's better-than-chance validation capacity implicitly, without any explicit verifier.
...
LATS applies MCTS-style search to agent trajectories, combining generation, evaluation, and backtracking. The paper reports improvements on programming tasks (HumanEval) and web navigation (WebShop), among others, demonstrating that systematic search with explicit value functions outperforms linear chain approaches. The method exemplifies how allocating more inference compute to validator-guided exploration narrows the GV gap.
...
This paper documents that pairwise LLM judge outcomes change substantially depending on which answer appears first (position bias): GPT-4 tends to prefer the first-position response, ChatGPT tends to prefer the second. Even when aggregate correlation with human preferences is high, these biases can reverse individual comparisons. The paper establishes that position randomization and bidirectional scoring (evaluating both orderings and combining results) are necessary minimum controls for any judge-mediated evaluation or training pipeline.

\paragraph{Judging the Judges (Shi et al., 2024) and PORTIA/Split-and-Merge (Li et al., 2024).}
Judging the Judges conducts a large-scale empirical study across 15 judge models and multiple evaluation settings, systematically mapping position bias and other reliability failure modes. PORTIA (Li et al.) proposes a different debiasing strategy: it \emph{splits} responses into aligned content segments, then merges segment-aligned prompts for fairer pairwise comparison---distinct from simple bidirectional A/B averaging. Both papers report substantial reductions in positional bias while maintaining evaluation utility. Together they provide practical debiasing protocols for pairwise evaluation pipelines.

\paragraph{CalibraEval (Li et al., 2024) and AlpacaEval (Dubois et al., 2024).}
CalibraEval addresses selection bias in LLM judge probabilities: observed token/probability distributions are distorted by positional and other biases, and calibration techniques applied to these distributions improve consistency and downstream evaluation performance. AlpacaEval (``A Simple Way to Debias Automatic Evaluators'') focuses specifically on \emph{length bias}: LLM judges systematically favor longer responses, and the paper proposes length-controlled win rates via a GLM model over preference pairs. Both papers address the problem that raw judge scores are biased and need correction before serving as reliable training signals.
...
Kuhn et al.\ show that semantic entropy---clustering sampled outputs by meaning before computing entropy---is a better \emph{predictor of model correctness} (higher AUROC) than token-level sequence entropy or P(True) baselines. The paper explicitly cautions against using standard calibration metrics for free-form NLG; the improvement is in uncertainty estimation, not calibration per se. The key implication is that how you aggregate uncertainty signals from generation samples---measuring meaning-space variance rather than token-level variance---changes the quality of the readout. Uncertainty is partly a readout design problem, not only a model capability problem.

\paragraph{Benchmarking and Improving Generator--Validator Consistency (Li et al., 2023).}
This Stanford paper directly measures and frames the GV gap, proposing a consistency fine-tuning approach where generator and validator learn from each other by filtering to consistent pairs:

\begin{paperquote}
\small ``[O]ur method improves the generator quality by 16\% and the validator accuracy by 6.3\%. \ldots\ [T]he skill of generator-validator consistency generalizes to unseen domains.''\\
\hfill --- Li et al.\ (2023)
\end{paperquote}

This is the paper that most directly names and operationalizes the problem this survey is organized around, alongside RankAlign.

\paragraph{RankAlign (Rodriguez et al., 2025).}
RankAlign defines the GV gap in terms of the \emph{correlation of generator and validator log-odds} over candidate completions, then introduces a ranking-based loss to align them:

\begin{paperquote}
\small ``We find this strategy significantly reduces the G-V gap across models by 31.8\% on average, giving substantially higher correlation between generator and validator \ldots\ Moreover, our approach generalizes well to unseen tasks, indicating that closing the G-V gap is a skill that transfers.''\\
\hfill --- Rodriguez et al.\ (RankAlign, 2025)
\end{paperquote}

As the central anchor paper for this literature survey, it provides both a precise diagnostic (log-odds correlation) and a practical training-level intervention.

\paragraph{Post-training effects: InstructGPT, DPO.}
InstructGPT (Ouyang et al.) shows that preference tuning substantially narrows the gap between model output and human expectations---a 1.3B InstructGPT model is preferred over the 175B base GPT-3 on most evaluated prompts. The paper also documents that InstructGPT shows improvements in truthfulness and toxicity reduction, though the precise effects on generation--validation consistency are not directly quantified in that work. DPO (Rafailov et al.) achieves comparable preference-optimization gains with a simpler, PPO-free objective, as documented in Q1. The clearest path to origin-consistent post-training remains process/verifier supervision (Cobbe; Uesato; Lightman) that provides explicit credit assignment at the step level, rather than outcome-only preference optimization.
